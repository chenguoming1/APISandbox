...
import { Network, FileText, CheckCircle2, Terminal, ShieldAlert, AlignLeft, Globe, ChevronDown, ChevronUp, HelpCircle } from 'lucide-react';
...
export default function ResponseViewer({ 
  ...
  const testPassedCount = response?.tests.filter(t => t.passed).length || 0;
  const testFailedCount = response?.tests.filter(t => !t.passed).length || 0;
  ...
  const statusColor = response
    ? response.status >= 200 && response.status < 300 
      ...
      : response.status >= 400 
        ? 'text-rose-455 bg-rose-955/50 border border-rose-900/60 font-semibold'
    ...
    >
        ...
        <div className="flex items-center gap-4">
          ...
          {response ? (
            ...
            <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">
              {isLoading ? 'Connecting Gateway...' : 'Console Gate Offline'}
            </span>
          )}
        </div>

        {/* Right header controls and toggles */}
        <div className="flex items-center gap-3">
          ...
          {response && !isCollapsed && (
            <div className="flex items-center gap-1">
              ...
              >
                ...
                <span className="text-[9px] bg-slate-800 text-slate-300 px-1 hover:bg-slate-700 rounded ml-1 font-mono">
              ...
              >
                ...
                {response.tests.length > 0 && (
                  <span className={`text-[9.5px] px-1.5 py-0.2 rounded font-mono font-bold ml-1 ${
                    testFailedCount > 0 ? 'bg-rose-950/60 text-rose-455' : 'bg-emerald-950/60 text-emerald-400'
                  }`}>
          ...
          {onToggleCollapse && (
            <button
              ...
              className="text-slate-450 hover:text-indigo-400 p-1 rounded hover:bg-slate-800 transition cursor-pointer flex items-center gap-1 flex-shrink-0"
              title={isCollapsed ? "Expand response details panel" : "Minimize response details panel"}
            >
              {isCollapsed ? (
                <>
                  <ChevronDown className="w-3.5 h-3.5" />
                  <span className="text-[10px] font-bold uppercase tracking-wider">Expand Page</span>
                </>
              ) : (
                <>
      ...
      {!isCollapsed && (
        <div className="flex-1 overflow-y-auto min-h-0 bg-slate-950/40 p-4">
          {isLoading ? (
            <div id="response_loader" className="h-full min-h-[180px] flex flex-col items-center justify-center text-slate-400 text-center py-12 select-none">
              <div className="w-8 h-8 rounded-full border-2 border-indigo-500 border-t-transparent animate-spin mb-3"></div>
              <p className="text-xs font-medium animate-pulse">Awaiting API Server Return Payload...</p>
              <p className="text-[10.5px] text-slate-500 mt-1 max-w-sm">Executing pre-request script and passing through CORS proxy gate...</p>
            </div>
          ) : !response ? (
            <div id="empty_response" className="h-full min-h-[180px] flex flex-col items-center justify-center text-slate-500 text-center select-none py-12">
              <Network className="w-10 h-10 text-slate-800 mb-2.5 animate-pulse" />
              <p className="text-xs font-semibold text-slate-400">Response Console is Empty</p>
              <p className="text-[10.5px] text-slate-500 mt-1 max-w-xs leading-relaxed">Configure items, customize scripts and trigger "Send Request" above to visualize RESTful resources.</p>
            </div>
          ) : (
            <>
              {/* BODY PANEL */}
              {activeTab === 'body' && (
                <div className="flex flex-col h-full gap-3">
                  ...
                  <div className="flex items-center justify-between border-b border-slate-900 pb-2 mb-1">
                    <span className="text-[10px] text-slate-550 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                      <FileText className="w-3.5 h-3.5" /> Content Payload Representation
                    </span>
                    <div className="flex bg-slate-900/60 p-0.5 rounded-md border border-slate-850">
                      ...
                      <button
                        ...
                        className={`px-2 py-0.5 rounded text-[10px] font-bold transition cursor-pointer ${
                          bodyFormat === 'raw' ? 'bg-indigo-600/30 text-indigo-400' : 'text-slate-500 hover:text-slate-250'
                        }`}
                      >
                        RAW
                      </button>
                      ...
                      <tbody>
                        {Object.entries(response.headers || {}).map(([key, value]) => (
                          <tr key={key} className="border-b border-slate-900 hover:bg-slate-900/20">
                            <td className="py-2.5 px-4 font-mono text-xs text-indigo-455 font-medium break-all select-all select-all-text select-text align-top">{key}</td>
                            <td className="py-2.5 px-4 font-mono text-xs text-slate-350 break-all select-all select-all-text select-text align-top">{value}</td>
                          </tr>
                        ))}
              ...
              {activeTab === 'tests' && (
                <div className="flex flex-col gap-4">
                  ...
                  <div className="space-y-2">
                    {response.tests.map((test, index) => (
                      ...
                      >
                        ...
                        <div className="flex-1">
                          ...
                          {test.error && (
                            <p className="font-mono text-[11px] text-rose-450 mt-1 pl-2 border-l border-rose-500 bg-rose-950/10 py-1.5 rounded font-semibold">
                    ...
                    {response.tests.length === 0 && (
                      <div className="flex flex-col items-center justify-center p-12 bg-slate-900/20 border border-slate-900 rounded-xl text-center flex-1">
              ...
              {activeTab === 'console' && (
                <div className="flex flex-col h-full bg-slate-950 border border-slate-900 rounded-lg overflow-hidden">
                  ...
                  <div className="flex-1 overflow-auto p-4 font-mono text-[11px] leading-5 text-slate-350 space-y-1.5 select-text min-h-[160px]">
                    {response.logs.map((log, index) => {
                      let textClass = 'text-slate-400';
                      if (log.includes('[Execution Error]') || log.includes('[Test Fail]')) {
                        textClass = 'text-rose-455 font-semibold';
